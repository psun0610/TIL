...
FROM healthcare
WHERE blood_pressure != '' and blood_pressure >= 200
...
> 키는 미터 단위로 계산한다.
```sql
SELECT is_drinking, smoking, AVG(weight/POWER((height*0.01), 2)) AS BMI FROM healthcare WHERE is_drinking != '' and smoking != '' GROUP BY is_drinking, smoking;
```
```
